...
public class ModelProvider {
    ...
    public static ResponseData getResponseModel(ResponseData response, String data)
            throws MhealthException {
        try {
            JSONObject jObj = new JSONObject(data);
            response.setResponseType(jObj.getString(KEY.RESPONSE_TYPE));
            response.setResponseName(jObj.getString(KEY.RESPONSE_NAME));
            response.setResponseCode(jObj.getString(KEY.RESPONSE_CODE));
            response.setExecTime(jObj.getString(KEY.RESPONSE_EXECT_TIME));
            if (response.getResponseCode().equalsIgnoreCase("00")) {
                response.setErrorCode(jObj
                        .getString(KEY.RESPONSE_ERROR_CODE));
                response.setErrorDesc(jObj
                        .getString(KEY.RESPONSE_ERROR_DESC));

                if (jObj.has(KEY.PARAM1)) {
                    response.setParam(jObj.getString(KEY.PARAM1));
                    response.setParamJson(jObj.getJSONObject(KEY.PARAM1));

                }
            } else {
                response.setDataLength(jObj.getInt(KEY.RESPONSE_DATA_LENGTH));
                response.setData(jObj.getString(KEY.RESPONSE_DATA));
                response.setDataJson(jObj.getJSONObject(KEY.RESPONSE_DATA));
                if (jObj.has(KEY.PARAM1)) {
                    response.setParam(jObj.getString(KEY.PARAM1));
                    response.setParamJson(jObj.getJSONObject(KEY.PARAM1));
    ...
    public static Beneficiary getBeneficiaryModel(Beneficiary beneficiary,
        ...
        for (String key : questionAnswers.keySet()) {
             ...
             */
                ...
                beneficiary.setGuardianName(engName);
                beneficiary.setGuardianLocalName(localName);
            } else if (qName != null
                ...
                try {
                    ...
                    && qName.trim().equalsIgnoreCase("AGREED_MOBILE_COMM")) {
                beneficiary.setAgreedMobileComm(qAnswer);
            } else if (qName != null
                    && qName.trim().equalsIgnoreCase("MOBILE_COMM_LANG")) {
                beneficiary.setMobileCommLang(qAnswer);
            } else if (qName != null
                    && qName.trim().equalsIgnoreCase("HH_CHARACTER")) {
                beneficiary.setHhCharacter(qAnswer);
            } else if (qName != null
                    && qName.trim().equalsIgnoreCase("HH_NAME")) {
                beneficiary.setHhName(qAnswer);
            } else if (qName != null
...
//					doctorFeedback.setRefCenterId(jsonObject.getLong(Column.REF_CENTER_ID));
    ...
    public static MyData getMyData(Context context, MyData myData, ResponseData response)
            throws MhealthException {
        try {
            if (response.getDataJson() != null) {
                myData.setResponseInfo(response);
                JSONObject jDataObj = response.getDataJson();

                // ///// Referral center list///////////////
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.REFERRAL_CENTERS)) {
                ...
                // ///// Questionnaire List//////
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.QUESTIONNAIRES)) {
                ...
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.CATEGORIES)) {
                ...
                // /////// Beneficiary List///////////////
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.BENEFICIARIES)) {
                ...
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.HOUSEHOLDS)) {
                ...
                // /// Medicine List//////////////
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.MEDICINES)) {
                ...
                // ///////// FCM Profile/////////////
                if (response.getResponseName().equals(RequestName.MY_DATA)
                        || response.getResponseName().equals(RequestName.FCM_PROFILE)) {
                ...
                // ///////// App Version History /////////////
                if (response.getResponseName().equals(RequestName.APP_VERSION_HISTORY)) {
                ...
                // ///////// Schedule List /////////////
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.USER_SCHEDULE)
                        || response.getResponseName().equals(RequestName.UNDONE_SCHEDULE)) {
                ...
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.CCS)) {
                ...
                if (response.getResponseName().equals(RequestName.MY_DATA_PARAMEDIC)) {
                ...
                if (response.getResponseName().equals(RequestName.COURT_YARD) || response.getResponseName().equals(RequestName.MY_DATA)) {
...
//
                ...
                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.IMMUNIZATION)) {
                ...
                // /// Medicine ConsumptionList//////////////
//                if (response.getResponseName().equals(RequestName.MY_DATA) || response.getResponseName().equals(RequestName.MEDICINE_CONSUMPTION)) {
//                    if (jDataObj.has(DBTable.)) {
//                        myData.setMedicineList(JSONParser.parseMedicineListJSON(jDataObj.getJSONArray(DBTable.MEDICINE)));
//                    }
//
//                    if (jDataObj.has(KEY.MEDICINES_STOCK)) {
...
//                }
                ...
                // /////////////////////////////////////
                myData.setVersionNos(response.getParamJson());
    ...
    public static ContentValues getCoupleRegistration(Beneficiary beneficiary, HashMap<String, QuestionAnswer> questionAnswers, long interviewId, long startTime,String coupleCode) {
        if (questionAnswers.size() == 0) return null;
        ContentValues cv = new ContentValues();
        cv.put("BENEF_ID", beneficiary.getBenefId());
        cv.put("BENEF_CODE", beneficiary.getBenefCode());
        cv.put("REG_DATE", startTime);
        cv.put("COUPLE_CODE", coupleCode);
        ...
        for (String key : questionAnswers.keySet()) {
            ...
            if (qName != null && !qName.trim().equalsIgnoreCase("")) {
                if (qName.trim().equalsIgnoreCase("WIFE_BENEF_CODE")) {
                    cv.put("WIFE_BENEF_CODE", qAnswer);
                } else if (qName.trim().equalsIgnoreCase("HUSBAND_BENEF_CODE")) {
                    cv.put("HUSBAND_BENEF_CODE", qAnswer);
                } else if (qName.trim().equalsIgnoreCase("MARRIAGE_DATE")) {
                    try {
                        cv.put("MARRIAGE_DATE", qAnswer);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                } else if (qName.trim().equalsIgnoreCase("AGE_FIRST_PREGNENCY")) {
                    cv.put("AGE_FIRST_PREGNANCY", qAnswer);
                } else if (qName.trim().equalsIgnoreCase("NO_OF_LIVING_CHILDREN")) {
                    cv.put("LIVING_CHILDREN", qAnswer);
                } else if (qName.trim().equalsIgnoreCase("TT_INFO")) {
                    cv.put("TT_INFO", qAnswer);
                }
            }
        }
        return cv;
