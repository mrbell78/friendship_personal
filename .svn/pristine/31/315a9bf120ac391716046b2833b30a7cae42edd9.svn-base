...
public class UploadInterviewTaskEarlierService extends AsyncTask<Void, Void, String> {

	/** The selected interview lis. */
	ArrayList<InterviewInfoSyncUnsync> selectedInterviewLis;
	 ...
	 */
	public UploadInterviewTaskEarlierService(Context ctx, ArrayList<InterviewInfoSyncUnsync> selectedInterviewLis, String moduleType)
	{
...
//  }
	...
	protected String doInBackground(Void params) throws Exception {
		...
		{
			InterviewInfoSyncUnsync savedInterview = selectedInterviewLis.get(i);
			...
			if(responseData.getWebResponseStatusCode() == 200)
			{
				...
				if(responseData.getResponseCode().equalsIgnoreCase("01"))
				{
					...
					App.getContext().getDB().updateInterviewSentService(savedInterview.getInterviewId(), savedInterview.getParentInterviewId(), "Y", affectedRow);
