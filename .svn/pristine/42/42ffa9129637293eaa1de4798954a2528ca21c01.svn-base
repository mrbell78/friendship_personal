...
import java.util.*
import ngo.friendship.satellite.BR

/*
 * Author: Md. Yeasin Ali
 * created: 23 May 2022
 * */
abstract class BasemHealthAdapter<T, VB> : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        onBindData(
                mModelList[position],
                position,
                (holder as BasemHealthAdapter<*, *>.DataViewHolder).mDataBinding as VB
        )
    ...
    override fun getItemCount(): Int =
            if (size == null || mModelList.size <= size!!) mModelList.size else size!!
